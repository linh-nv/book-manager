    ...
    <div class="head-content flex justify-between items-center w-full">
      ...
      <div class="flex justify-between items-center gap-20 ml-20">
        ...
        <div class="relative">
          ...
          >
            <div
              v-for="category in limitedSearchResults"
              :key="category.id"
              @click="showItem(category.id)"
...
const isListPage = ref(route.name === "category-list");

const navigateToCreate = () => {
  router.push({ name: "category-form" });
};
...
watch(route, (newRoute) => {
  isListPage.value = newRoute.name === "category-list";
});
...
const searchItems = async () => {
  try {
    const data = await apiService.search("/category", searchKeyword.value);
