    ...
    <div class="head-content flex justify-between items-center w-full">
      ...
      <div class="flex justify-between items-center gap-20 ml-20">
        ...
        <div class="relative">
          ...
          >
            <div
              v-for="lendTicket in limitedSearchResults"
              :key="lendTicket.id"
              @click="showItem(lendTicket.id)"
            ...
            >
              ...
              <p class="overflow-hidden text-ellipsis whitespace-nowrap">
                {{ lendTicket.start_date }} - {{ lendTicket.end_date }} - {{ getStatusLabelById(lendTicket.status) }}
              </p>
            </div>
          </div>
        </div>
...
const isListPage = ref(route.name === "lend-ticket-list");

const navigateToCreate = () => {
  router.push({ name: "lend-ticket-form" });
};
...
watch(route, (newRoute) => {
  isListPage.value = newRoute.name === "lend-ticket-list";
});
...
const searchItems = async () => {
  try {
    const data = await apiService.search("/lend-ticket", searchKeyword.value);
    searchResults.value = data.data;
  } catch (error) {
    console.error("Error searching lendtickets:", error);
  }
};
